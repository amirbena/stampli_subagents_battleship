...
| `gitignore-compliance-policy.md` | Files matching `.gitignore` must not be staged or committed; `package-lock.json` may remain changed locally but must not be staged or pushed; defines verification commands and enforcement points for release-pr-agent and code-review-agent | release-pr-agent (pre-commit gate), code-review-agent (review checklist), Team Lead (QA loop routing) |
